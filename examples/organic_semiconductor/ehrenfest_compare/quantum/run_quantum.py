...
sample_time = 40 * Constants.fs
sample_freq = int(Constants.fs/dt)
nsteps = int(sample_time/dt)
temperature = args.temperature

## hopping parameters
hopping_in_cm = - 100   # cm^-1
hopping_coef = hopping_in_cm * Constants.cm_inverse_energy
## boson bath parameters
num_modes = 1
nmax = args.nmax
omega_in_cm = 1000.0 # cm^-1;   1000cm^-1 --> ~124meV  ~ 1500K
...
print('Hopping coefficient [cm^-1]:', hopping_in_cm, 'in internal units:', hopping_coef)
print('g factor:', g_factor)
print('omega [cm^-1]:', omega_in_cm, 'in internal units:', omega)
print('polaron binding energy [cm^-1]:', g_factor**2 * omega_in_cm)
"""
initiate the system
"""
...
### processing
prefix = 'ns{}_nmax{}_T{:.0f}_V{}_g{:.1f}_dt{:.3f}'.format(ns, nmax, temperature, hopping_in_cm, g_factor, dt/Constants.fs)
...
plt.tight_layout()
plt.savefig('{}.png'.format(prefix), dpi=200)
plt.close()
