...
nmax = np.array([6, 3, 1, 1, 1, 1, 1, 1, 1])
keep_modes = 2
...
rho0_list = [th.tensor([[0.5, 0.5], [0.5, 0.5]])]
...
for i in range(num_modes):
    ...
    system_harmonic = ComposedOperatorGroups(id="system_harmonic_{}".format(i), OP_list=system_harmonic_list, static=True)
...
for i in range(num_modes):
    ...
    system_couple = ComposedOperatorGroups(id="system_couple_{}".format(i), OP_list=system_couple_list, static=True)
...
for i in range(num_modes):
    ...
    boson_up_jump = ComposedOperatorGroups(id="boson_up_jump_{}".format(i), OP_list=boson_up_jump_list, static=True)
    boson_dn_jump = ComposedOperatorGroups(id="boson_dn_jump_{}".format(i), OP_list=boson_dn_jump_list, static=True)
...
obs_system_sz = ComposedOperatorGroups(id="obs_system_sz", OP_list=[obs_spin_sz]+boson_identity_list, static=True)

obs_spin_sx = StaticPauliOperatorGroup(n_qubits=1, id="obs_spin_sx", batchsize=batchsize, coef=1.0)
obs_spin_sx.add_operator('X')
obs_system_sx = ComposedOperatorGroups(id="obs_system_sx", OP_list=[obs_spin_sx]+boson_identity_list, static=True)
...
obs_system_excited = ComposedOperatorGroups(id="obs_system_excited", OP_list=[obs_spin_excited]+boson_identity_list, static=True)
...
for i in range(num_modes):
    ...
    obs_boson_N_list.append(
        ComposedOperatorGroups(id="obs_boson_N_{}".format(i), OP_list=obs_boson_N_op_list, static=True)
        )
...
ax[1].legend()
plt.savefig('ne_tau{:.2f}ps_dt{:.3f}fs.png'.format(tau_in_ps, dt/Constants.fs), dpi=300)
